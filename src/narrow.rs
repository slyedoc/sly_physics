...
pub fn narrow_system(
    ...
    for pair in broad_contacts.iter() {
        unsafe {
            let (mut trans_a, type_a, lin_vel_a, mut ang_vel_a, com_a, i_tensor_a, mesh_handle_a) =
                query.get_unchecked(pair.a).unwrap();
            let (mut trans_b, type_b, lin_vel_b, mut ang_vel_b, com_b, i_tensor_b, mesh_handle_b) =
                ...
                (collider_a, collider_b) => {
                    ...
                    while dt > 0.0 {
                        ...
                        if let Some((mut world_point_a, mut world_point_b)) =
                            gjk_does_intersect(&collider_a, &trans_a, &verts_a, &collider_b, &trans_b, &verts_b, BIAS)
                        {
                            ...
                            contacts.send(Contact {
                                ...
                                world_point_a,
                                world_point_b,
                                local_point_a: RBHelper::world_to_local(
                                    &trans_a,
                                    com_a,
                                    world_point_a,
                                ),
                                local_point_b: RBHelper::world_to_local(
                                    &trans_b,
                                    com_b,
                                    world_point_b,
                                ),
                        ...
                        let (world_point_a, world_point_b) = gjk_closest_points(&collider_a, &trans_a, &verts_a, &collider_b, &trans_b, &verts_b);
